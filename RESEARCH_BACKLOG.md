...
| 19 | TOK_entropy_driven_pretokenization_boundary | C30 1510Z — arXiv:2506.15889 (PMI + L/R entropy) | place pre-tokenization boundaries via PMI + left/right entropy from FineWeb BEFORE BPE merge phase; complementary to entropy-aware merge ordering by shifting decision earlier | -0.008 train_loss vs vanilla BPE-8192 | world-novel-candidate (needs source verification) | 95 | 20260408T1510Z |
| 20 | TOK_byteflow_adaptive_compression_hybrid | C30 1510Z — ByteFlow 2026 (source citation needs verification) | 2-layer hierarchical tokenizer (byte→semantic→vocab-1024); learns soft adaptive merge boundaries with decoder-side reconstruction loss; entropy clustering vs frequency-driven | -0.012 BPB indirect | comp-novel (source uncertain) | 180 | 20260408T1510Z |
| 21 | TOK_tree_structured_vocab_factorization | C30 1510Z — tree-structured diffusion adaptation | vocab as prefix tree: 512 high-entropy core + 512 leaf expansion nodes for rare contexts; 2-stage hierarchical softmax; 80% of tokens skip full-vocab logit | -0.006 BPB (20% faster forward + quality) | world-novel-candidate (untested for byte LM) | 140 | 20260408T1510Z |
...
| 16 | EMB_token_gating_adaptive_carvq | C30 1510Z — CARVQ EMNLP 2025 + Jina spherical compress | learned per-token gate routes common bytes through 8-bit quantized embeddings, rare bytes through low-rank factorized paths; CARVQ-inspired adaptive routing for INT6 regime | -0.004 BPB via 0.8MB freed for ngram tables | comp-novel | 85 | 20260408T1510Z |
| 17 | EMB_residual_vector_quantization_rvqt | C30 1510Z — CARVQ + Soundstream/EnCodec audio codec RVQ | 4-level group residual VQ on tok_emb (each level 4-bit codebook); hierarchical residual coding captures within-group structure better than single K-means; tied head reconstructs via learned residual projection | -0.005 BPB via 0.9MB freed | comp-novel (RVQ known; byte-vocab 4-level tying is the novelty) | 95 | 20260408T1510Z |
